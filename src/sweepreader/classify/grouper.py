...
def _build_prompt(batch: list[tuple[Group, list["Item"]]]) -> str:
    lines = [
        ...
        "",
        "  3. Parallel SRO filings by sibling exchanges — an exchange group files the",
        "     SAME rule change separately for each of its registered exchanges, so the",
        "     Federal Register carries one near-identical notice per entity (MIAX,",
        "     MIAX Pearl, MIAX Emerald, MIAX Sapphire; or Nasdaq ISE, MRX, GEMX, PHLX;",
        "     or Cboe, C2, BZX, EDGX). Each has its own SR- number and its own legal",
        "     filer, and the entity name appears in the title — but it is ONE",
        "     regulatory action and a desk wants to read it once. Treat these as the",
        "     same event when the entities belong to one exchange group, the notices",
        "     are same-day, and the substance (the rule, fee or mechanism being",
        "     changed) is the same. Differing SR- numbers and filer names are expected",
        "     here and are NOT grounds to split them.",
        "",
