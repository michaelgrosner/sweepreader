...
**3.1c BOX notices feed.** ✅ Source `box_notices` (`parse: rss_generic`, enabled). BOX migrated to WordPress; notices are a `circulars` custom post type exposed as RSS at `boxexchange.com/?post_type=circulars&feed=rss2`. The HTML `/notices` listing is Cloudflare-challenged, but the feed (and sitemap) paths are not. Notices are **title-only** (empty `entry-content`, no PDF) — the headline is the notice; substantive detail is in the 19b-4 circulars already covered by the Federal Register — so no detail fetch is needed and the generic RSS adapter suffices (just a `box_notices→BOX` venue mapping). Supersedes the disabled `email_box`. Deep seeding skipped: the feed holds only recent items and the sitemap's lastmod is a bulk-import date, so historical BOX adds little beyond Federal Register filing history.

**3.2 Historical seed CLI (backtesting).** ✅ `sweepreader seed [--months 6] [--source ...]` (`cli/seed.py`). Pages each seedable source's history back to the cutoff and appends to the append-only store with `first_seen_at = published_at`, so the seeded past reconstructs faithfully under time-travel and backtest (SPEC §5). Idempotent (re-runs add zero). Classification is left to `run`/`backtest` so only uncached combos cost tokens. NYSE seeds from its API (bodies inline). MIAX walks `?page=N` (≈1.5 pages/day) using **teaser-first + lazy body**: each item starts from the listing teaser and a detail page is fetched only when a cheap token-free keyword gate accepts it (tier-E noise stays teaser-only) — in a 5-day sample this skipped ~60% of detail fetches. Flags: `--all-bodies`, `--body-min-relevance`, `--no-cache`.
